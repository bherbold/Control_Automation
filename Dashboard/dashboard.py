...
for seconds in range(len(df)):
    ...
    with placeholder.container():
        ...
        st.subheader(
            '2) On the first tab you will find operational conditions, on the second tab you will find an economic overview')
        ...
        with tab1:
            ...
            if seconds > 0:
                delta = float(
                    df["Power_W"][seconds] * df["ON/OFF"][seconds] - df["Power_W"][seconds - 1] * df["ON/OFF"][
                        seconds - 1])
            else:
                delta = 0
        ...
        with tab2:
            ...
            value21 = int(
                schedule_df['Estimated Emissions [ton/MWh]'][int(str(datetime.now()).split(' ')[1].split(':')[0])])
